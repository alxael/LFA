...
public:
    explicit DFA(int initialState = 0, string lambdaString = "-") : initialState(initialState),
                                                                    lambdaString(lambdaString) {}

    friend istream &operator>>(istream &in, DFA &a) {
        int n, m, nrf, x, y;
        char l;
        in >> n;
        for (int i = 1; i <= n; i++) {
    ...
    friend ostream &operator<<(ostream &out, DFA &a) {
        out << "States: ";
...
int main() {
    ...
    for (int index = 0; index < nrWords; index++) {
        in >> word;
        vector<int> solution = dfa.evaluateStringWithPath(word);
        if (!solution.empty()) {
            cout << "VALID" << endl;
